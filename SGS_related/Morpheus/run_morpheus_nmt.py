...
import argparse
from SGS_related.Morpheus.morpheus_nmt import MorpheusFairseqTransformerNMT
...
with open(src_file, 'r') as src_stream, open(tgt_file, 'r') as tgt_stream:
    ...
    for src, ref in zip(src_stream, tgt_stream):
        ...
        adv, adv_pred, is_attack_success, query_number, modif_rate = morpheus.morph(src, ref)
...
print(f'skip number: {skip_num}, all number:{all_num}')
all_loggers.summary()
