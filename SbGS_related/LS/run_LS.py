...
from SbGS_related.LS.adversarial_tools_ls import adversarial_paraphrase_LS
...
def fool_text_classifier():
    ...
    # my_file.create_folder(PWWS_OUT_PATH, result_folder)
    # log_file = open(my_file.real_path_of(PWWS_OUT_PATH, result_folder, 'log.txt'), 'a')
    ...
    # Read data set
    train_texts, train_labels, test_texts, test_labels, x_train, y_train, x_test, y_test = load_dataset(dataset,
                                                                                                        args.level)
    ...
    for index in test_indices:
        ...
        adv_doc, adv_y, modi_rate = adversarial_paraphrase_LS(input_text=text,
                                                              true_y=np.argmax(y_test[index]),
                                                              grad_guide=grad_guide,
                                                              tokenizer=tokenizer,
                                                              dataset=dataset,
                                                              level=args.level)
        query_num = grad_guide.end_attack()
