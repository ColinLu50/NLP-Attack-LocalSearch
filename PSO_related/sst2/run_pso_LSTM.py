...
attcker = PSOAttack(model, word_candidate, dataset,
                    max_iters=max_iter,
                    pop_size=pop_size)
...
for test_idx in range(all_test_num):
    ...
    x_adv = attcker.attack(x_orig, target_label, pos_tags)

    cur_logger.log_result(x_adv, x_orig, x_len, attcker)
    ...
    #         adv_orig.append(test_idx)
    #         adv_orig_label.append(orig_label)
    #         adv_training_examples.append(test_idx)
