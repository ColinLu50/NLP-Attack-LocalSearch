...
class PSOAttack(object):
    ...
    def perturb(self, x_cur, x_orig, neigbhours, w_select_probs, target):
        ...
        tmp1 = np.array(x_cur)
        tmp2 = np.array(x_orig)

        while x_cur[rand_idx] != x_orig[rand_idx] and np.sum(tmp1 != tmp2) < np.sum(np.sign(w_select_probs)):
    ...
    def attack(self, x1, x_orig, target, pos_tags):
        ...
        self.query_num_cached = 0
        self.invoke_dict = {}
        pop_x1 = self.self_copy(x1, self.pop_size) # what for?
        self.premise = x1
