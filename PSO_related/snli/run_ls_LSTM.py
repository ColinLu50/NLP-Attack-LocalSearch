...
from PSO_related.snli.attack_ls_snli import LSAttack

from PSO_related.snli.model_nli import Model
...
word_vec_path = os.path.join(SNLI_data_folder_path, 'word_vec.pkl')
glove_path = os.path.join(SNLI_data_folder_path, 'glove_unk.pkl')
nli_net_path = os.path.join(SNLI_data_folder_path, 'savedir')

model = Model(word_vec_path, glove_path, nli_net_path)
...
for test_idx in test_idxs:
    ...
    if attack_pred != true_label:
        ...
        # attack
        attack_result = attacker.attack(s1, s2, target, pos_tags)

        cur_logger.log_result(attack_result, s2, x_len, attacker)
